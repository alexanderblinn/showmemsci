...
def build_figure(profile: pd.DataFrame) -> tuple[go.Figure, dict[str, str]]:
    ...
    for visible in (True, False):
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "%{x|%Y-%m-%d}<br>Index level %{y:,.2f}<extra></extra>"
                ),
                showlegend=False,
                visible=visible,
            )
    ...
    summary = {
        "span": (
            f"{profile.index.min():%Y-%m} to {profile.index.max():%Y-%m}"
        ),
...
def main() -> None:
    ...
    theme.render_html(
        fig,
        SAVE_HTML_TO,
        eyebrow="Single / Editorial Edition",
        ...
        kicker="One index path<br />two reading modes",
        meta_items=[
            summary["span"],
            summary["weeks"],
            summary["latest"],
        ...
        footer_left="Weekly first-close sample of the MSCI World index level.",
