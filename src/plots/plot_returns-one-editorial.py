"""
Create an editorial, professional-looking variant of the annual return bands chart.

The underlying chart type stays identical to the original Returns I graphic:
...
def build_figure(returns: pd.DataFrame) -> tuple[go.Figure, dict[str, str]]:
    ...
    for band_index, interval in enumerate(intervals):
        ...
        for stack_index, (year, row) in enumerate(band_df.iterrows()):
            ...
            note = YEAR_NOTES.get(
                year, "Calendar-year return sorted into its performance band."
            )
...
def main() -> None:
    ...
    theme.render_html(
        fig,
        SAVE_HTML_TO,
        eyebrow="Returns I / Editorial Edition",
        title="MSCI World Return Bands",
        deck=(
            "The same bucketed annual-return chart, translated into a more "
            "professional report language: quieter palette, cleaner hierarchy, "
            "and enough tonal contrast to keep the distribution readable at a glance."
        ),
        kicker=f"Calendar-year returns<br />sorted into {summary['bands']}",
        meta_items=[
            summary["years"],
            summary["bands"],
            summary["split"],
        ...
        footer_left="Each tile represents one calendar year placed into its annual-return bucket.",
