...
def build_figure(annualized: pd.DataFrame, total: pd.DataFrame) -> tuple[go.Figure, dict[str, str]]:
    """Build the themed return-triangle heatmap."""
    holding_columns = [column for column in annualized.columns if column.startswith("Return_")]
    ...
    theme.apply_to_figure(
        ...
        xaxis=dict(
            title=dict(text="Holding period in years", font=dict(size=12, color=theme.MUTED)),
            hoverformat=".0f",
...
def main() -> None:
    ...
    theme.render_html(
        fig,
        SAVE_HTML_TO,
        eyebrow="Heatmap / Editorial Edition",
        ...
        kicker="Rows = start years<br />columns = holding spans",
        meta_items=[
            summary["span"],
            summary["horizon"],
            summary["annualized"],
        ...
        footer_left="Toggle between annualized and total return readings for the same return triangle.",
