...
def build_figure(returns: pd.DataFrame) -> tuple[go.Figure, dict[str, str]]:
    ...
    lower = returns.min(axis=0).mul(100)
    upper = returns.max(axis=0).mul(100)
    average = returns.mean(axis=0).mul(100)
...
def main() -> None:
    ...
    theme.render_html(
        fig,
        SAVE_HTML_TO,
        eyebrow="Long-term / Editorial Edition",
        ...
        kicker="Best, worst, average<br />for every holding span",
        meta_items=[
            summary["horizon"],
            summary["start"],
            summary["one_year"],
        ...
        footer_left="Each point summarizes all rolling investment windows of that length.",
